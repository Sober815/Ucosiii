...
int main()
{
	...
	LCD_ShowString(30,90,200,16,16,"2021/9/2..");
    
    
    //初始化UCOSIII
    OSInit(&err);
    //先进入临界区
    OS_CRITICAL_ENTER();//用这个函数要加cpu_SR_alloc
    ...
    OSTaskCreate(
                    ...
                    (CPU_STK    *)   START_TASK_STK,       //任务堆栈基地址&START_TASK_STK[0]
                    (CPU_STK_SIZE)   START_TASK_SIZE/10,//任务堆栈深度限位
                    (CPU_STK_SIZE)   START_TASK_SIZE,     //任务堆栈大小
                    (OS_MSG_QTY)     0,         //任务内部消息队列能够接收的最大消息数目,为0时禁止接收消息
                    (OS_TICK)        0,         //当使能时间片轮转时的时间片长度，为0时为默认长度，1=5ms
                    (void       *)     0,           //用户补充的存储区
